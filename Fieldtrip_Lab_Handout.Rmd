...
Open the .csv sheet & the tibble and compare - does it make sense to you?
...
Finally, with coding, it can be very easy to lose sight of what you are actually doing to the data itself. To avoid this, we recommend regularly viewing your dataset, either by exporting it and opening it in Excel, or simply by printing the 'Data' variable. You can do this either by simply entering the name of the variable you want to look at in the console, by entering the following line in the console to export the file, or by opening the variable you want to investigate in the 'Environment' tab to the right. 
...
Because we have an internal standard (as most quantitative LC-MS data will), we want to use the ratio of our analyte peak area to our internal standard peak area. Write some R code to create a new column of the CalData table containing this ratio, called 'ratio'.  The `mutate` function (Chapter 12.5) will be useful here. 

```{r}
CalData <- #Your code to calculate the analyte to internal standard peak area ratio
```

For the purposes of calculating LOD/LOQ, we want a linear calibration curve. Start by plotting the calibration curve for one analyte, then select the appropriate concentration range to make it linear, filter out concentrations greater than that value, and plot the filtered data again. again. The calibration dataset contains the concentrations in the Sample.name column, which we can extract using the `separate` function (Chapter 11.2.2). Plotting is performed using ggplot (see Chapter 13)
...
linear <- 100 #This defines the highest value concentration we want to use for the linear portion of this calibration curve
...
#Write code to make a plot of instrument response vs concentration for one analyte for the filtered dataset
